...
def test_extract_game_status():
    ...
    assert football_db.extract_game_status("(12/14) <b>Questionable</b> @ KC") == "questionable"
    assert football_db.extract_game_status("--") is None
    assert football_db.extract_game_status("random text") is None

@patch("scrape_and_score.scraping.football_db.fetch_pks_for_inserted_player_injury_records")
...
@patch("scrape_and_score.scraping.football_db.player_service.get_player_id_by_normalized_name")
...
@patch("scrape_and_score.scraping.football_db.generate_and_persist_player_injury_records")
def test_scrape_upcoming(mock_persist, mock_fetch_page, mock_get_config, sample_soup):
    ...
    mock_get_config.return_value = "https://www.footballdb.com/transactions/injuries.html?yr={}&wk={}&type=reg"
