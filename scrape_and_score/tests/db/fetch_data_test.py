...
@patch(
    "db.fetch_data.get_connection", side_effect=Exception("Database connection failed")
)
...
@patch(
    "db.fetch_data.get_connection", side_effect=Exception("Database connection error")
)
...
def test_fetch_one_team_game_log_returns_expected_game_log(mock_get_connection):
    expected_game_log = {"team_id": 2, "week": 14, "year": 2024}
    mock_connection = MagicMock()
...
def test_fetch_one_team_game_log_executes_expected_sql(mock_get_connection):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM team_game_log FETCH FIRST 1 ROW ONLY"
    )
...
def test_fetch_one_player_game_log_returns_expected_game_log(mock_get_connection):
    expected_game_log = {"player_id": 2, "week": 14, "year": 2024}
    mock_connection = MagicMock()
...
def test_fetch_one_player_game_log_executes_expected_sql(mock_get_connection):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM player_game_log FETCH FIRST 1 ROW ONLY"
    )
...
@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_recent_week_returns_expected_list(
    mock_get_connection,
):
    expected_player_game_log = {
        "player_id": 527,
...
@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_recent_week_executes_expected_sql(
    mock_get_connection,
):
...
@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_recent_week_calls_expected_functions(
    mock_get_connection,
):
    mock_connection = MagicMock()
...
@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_given_year_returns_expected_list(
    mock_get_connection,
):
    expected_player_game_log = {
        "player_id": 527,
...
@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_given_year_executes_expected_sql(
    mock_get_connection,
):
    ...
    mock_cursor.execute.assert_called_once_with(
        "SELECT * FROM player_game_log WHERE year=%s", (2024,)
    )


@patch("db.fetch_data.get_connection")
def test_fetch_all_player_game_logs_for_given_year_calls_expected_functions(
    mock_get_connection,
):
    mock_connection = MagicMock()
