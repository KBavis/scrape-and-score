...
def test_insert_players_calls_insert_player_expected_number_of_times(
    mock_insert_player,
):
...
@patch(
    "db.insert_data.insert_player",
    side_effect=Exception("Unable to insert the following player: Kyler Murray"),
)
...
@patch(
    "db.insert_data.get_connection",
    side_effect=Exception("Unable to fetch database connection"),
)
...
@patch(
    "db.insert_data.insert_team",
    side_effect=Exception("Unable to insert the following team: Team One"),
)
...
def test_insert_teams_returns_expected_list(mock_insert_team):
    teams = ["Team One", "Team Two"]
    expected_list = [
        {"team_id": 1, "name": "Team One"},
        {"team_id": 1, "name": "Team Two"},
    ]
...
@patch(
    "db.insert_data.get_connection", side_effect=Exception("Database connection failed")
)
