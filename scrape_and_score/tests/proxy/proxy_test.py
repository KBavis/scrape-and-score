...
from proxy_helper import (
    mock_proxies_response,
    mock_with_open_expired_cache,
    get_html_ip,
    mock_with_open,
    EXPECTED_PROXIES,
    URL,
...
def test_get_proxy_no_cache():
    ...
    with mock.patch("os.path.exists") as mock_exists, mock.patch(
        "proxy.proxy.fetch_proxies"
    ) as mock_fetch_proxies, mock.patch(
        "proxy.proxy.is_cache_expired"
    ) as mock_is_cache_expired, mock.patch(
        "proxy.proxy.get_random_proxy"
    ) as mock_get_random_proxy:
