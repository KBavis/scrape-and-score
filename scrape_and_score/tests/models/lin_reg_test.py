...
def test_create_training_and_testing_split(mock_train_test_split):
    mock_train_test_split.side_effect = lambda inputs, targets, test_size, random_state: (inputs[:2], inputs[2:], targets[:2], targets[2:])
...
def test_test_regressions(mock_create_split, mock_linear_regression):
    ...
    mock_regression.predict.return_value = np.array([11.0, 12.0])  # Consistent return value for all positions
    ...
    assert mock_regression.predict.call_count == 8 # four for training, four for testing
