...
@patch(
    "data.preprocess.fetch_data.fetch_independent_and_dependent_variables_for_mult_lin_regression"
)
...
def test_get_rankings_ratios_returns_expected_tuples():
    mock_qb_data = pd.DataFrame(
        data=[
            {"def_pass_rank": 32, "off_pass_rank": 1},
            {"def_pass_rank": 1, "off_pass_rank": 32},
        ]
    )
    mock_wr_data = pd.DataFrame(
        data=[
            {"def_pass_rank": 32, "off_pass_rank": 1},
            {"def_pass_rank": 1, "off_pass_rank": 32},
        ]
    )
    mock_te_data = pd.DataFrame(
        data=[
            {"def_pass_rank": 32, "off_pass_rank": 1},
            {"def_pass_rank": 1, "off_pass_rank": 32},
        ]
    )
    mock_rb_data = pd.DataFrame(
        data=[
    ...
    actual_qb_data, actual_rb_data, actual_wr_data, actual_te_data = (
        preprocess.get_rankings_ratios(
            mock_qb_data, mock_rb_data, mock_wr_data, mock_te_data
        )
    )
...
def test_transform_data_returns_expected_tuples():
    qb_df = pd.DataFrame(
        data=[
            {
                "avg_fantasy_points": 15.0,
                "pass_ratio_rank": 32.0,
                "fantasy_points": 17.7,
            }
        ]
    )
    te_df = pd.DataFrame(
        data=[
            {
                "avg_fantasy_points": 16.0,
                "pass_ratio_rank": 16.0,
                "fantasy_points": 18.7,
            }
        ]
    )
    wr_df = pd.DataFrame(
        data=[
            {"avg_fantasy_points": 17.0, "pass_ratio_rank": 1.0, "fantasy_points": 19.7}
        ]
    )
    rb_df = pd.DataFrame(
        data=[
            {
                "avg_fantasy_points": 15.0,
                "rush_ratio_rank": 32.0,
                "fantasy_points": 18.7,
            }
        ]
    )

    qb_data, rb_data, wr_data, te_data = preprocess.transform_data(
        qb_df, rb_df, wr_df, te_df
    )
...
def test_filter_data():
    ...
    assert len(filtered_df) < len(
        df
    ), "Filtered DataFrame should have fewer rows than input"
