...
def feature_importance(
    ...
    training_data_set: torch.utils.data.Dataset,
    position: str,
    device: str,
):
    """
    ...
    logging.info(
        f"Post Training: Determining feature signficance of our {position} model"
    )
    ...
    feature_importance = pd.DataFrame(
        {"Feature": inputs, "SHAP_Importance": np.abs(shap_values).mean(axis=0)}
