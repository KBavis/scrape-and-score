...
def predict(week: int, season: int, model: str):
    ...
    model_mapping = {"QB": qb_nn, "RB": rb_nn, "WR": wr_nn, "TE": te_nn}
    ...
    for position in positions:
        ...
        position_specific_df = nn_preprocess.add_missing_features(
            position_specific_df, position_features
        )
        ...
        predictions[position] = prediction.generate_predictions(
            position, week, season, model_mapping[position], players, X
        )
...
def upcoming(week: int, season: int):
    ...
    all_player_ids = [
        player_id for game in relevant_games for player_id in game["player_ids"]
    ]
    ...
    football_db.scrape_upcoming(
        week, season, all_player_ids
    )  # TODO (FFM-319): Validate this site is updated throughout week of the NFL game

    # scrape player betting odds & game conditions
    betting_pros.fetch_upcoming_player_odds_and_game_conditions(
        week, season, all_player_ids
    )
...
def results(week: int, season: int):
    ...
    if week == props.get_config("final-week"):
        if not utils.are_team_seasonal_metrics_persisted(
            season
        ) or not utils.are_player_seasonal_metrics_persisted(season):
...
def neural_network(should_train: bool, start_time: datetime):
    ...
    required_models = ["rb_model.pth", "qb_model.pth", "te_model.pth", "wr_model.pth"]
    global rb_nn, qb_nn, wr_nn, te_nn

    # check if models exists and  that we do not want to retrain models
    directory = "models/nn/{}"
    if (
        all(os.path.exists(directory.format(model)) for model in required_models)
        and should_train == False
    ):
        ...
        for position in positions:
            logging.info(
                f"Extracting {position} pre-processed data into training/testing data sets"
            )
            ...
            selected_features = nn_preprocess.feature_selection(
                position_specific_df, position
            )
            ...
            training_data_set = FantasyDataset(
                training_df[selected_features + ["fantasy_points"]]
            )
            testing_data_set = FantasyDataset(
                testing_df[selected_features + ["fantasy_points"]]
            )
            test_data_loader = DataLoader(
                testing_data_set,
                batch_size=TRAINING_CONFIGS[position]["Batch Size"],
                shuffle=False,
            )
