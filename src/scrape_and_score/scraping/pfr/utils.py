...
def add_common_game_log_metrics(data: dict, tr: BeautifulSoup):
    ...
    data["date"].append(
        game_date.text if game_date else tr.find("td", {"data-stat": "date"}).text
    )
    ...
    data["team"].append(
        team.text if team else tr.find("td", {"data-stat": "team_name_abbr"}).text
    )
    ...
    data["opp"].append(
        opp.text if opp else tr.find("td", {"data-stat": "opp_name_abbr"}).text
    )
...
def construct_player_urls(players: list, season: int, base_url: str = None):
    ...
    players_with_hashed_name = [
        player for player in players if player["hashed_name"] is not None
    ]
    players_without_hashed_name = [
        player
        for player in players
        if player["hashed_name"] is None and player["pfr_available"] == 1
    ]  # disregard players previously indicated to be unavailable
    ...
    if base_url is not None:
        if players_without_hashed_name:
            logging.warning(
                f"The following players will also be skipped as there is no hashed_name persisted for them:\n\t{players_without_hashed_name}"
            )

        return get_player_page_urls(players_with_hashed_name, base_url)

    # order players by last name first initial
    ordered_players = order_players_by_last_name(
        players_without_hashed_name
    )  # order players without a hashed name by last name first inital
...
def get_last_name_first_initial(player_name: str):
    ...
    if len(first_and_last) < 2:
        raise Exception(
            f"Unable to extract first inital of last name of the players name: {player_name}"
        )
...
def filter_update_team_game_logs(records: list):
    ...
    for record in records:
        ...
        if not persisted_team_game_log:
            logging.info(
                f"No 'team_game_log' persisted corresponding to PK=[{pk}]; appending as insertable record"
            )
        ...
        if is_team_game_log_modified(record, persisted_team_game_log):
            logging.info(
                f"'team_game_log' correspond to PK=[{pk}] has been modified; appending as updateable record"
            )
...
def log_disregarded_players(players: list):
    ...
    disregarded_players = [
        player["player_name"] for player in players if player["pfr_available"] == 0
    ]
    if disregarded_players is not None and len(disregarded_players) != 0:
        logging.warning(
            f"The following players will be disregarded due to being unavailable in Pro-Football-Reference: \n\n{disregarded_players}\n\n"
        )
