...
def scrape_and_persist(start_year: int, end_year: int):
    ...
    html = scraping_util.fetch_page(
        "https://www.ourlads.com/nfldepthcharts/archive/150/IND"
    )
...
def generate_and_persist_depth_chart_records(
    teams: list, season: int, week: int, is_update: bool
):
    """ "
    ...
    if is_update is False:
        logging.info(
            f"Inital seasonal player depth chart insertion flow invoked; Scraping & persisting first player, player_teams, depth_chart_position records for {season} season"
        )
    else:
        logging.info(
            f"Update Flow Invoked; Account for potential updates to depth_chart_position & player teams records"
        )
    ...
    for team in teams:
        ...
        html = scraping_util.fetch_page(
            f"https://www.ourlads.com/nfldepthcharts/depthchart/{team['acronym']}"
        )
        soup = BeautifulSoup(html, "html.parser")
        ...
        generate_player_depth_chart_positions(
            players, None, None, player_depth_chart_position_records, season, week
        )
        add_unique_player_records(unique_player_records, players)

        insert_player_records(
            unique_player_records, team, player_name_id_mapping, season
        )
        ...
        if is_update:
            upsert_player_teams_records(
                relevant_players, team_id, season, team, player_name_id_mapping, week
            )
            upsert_player_depth_chart_position_records(
                player_name_id_mapping, player_depth_chart_position_records, season
            )
        else:
            ...
            insert_player_depth_chart_position_records(
                player_name_id_mapping, player_depth_chart_position_records, season
            )


def generate_player_and_player_teams_records(
    teams: list, start_year: int, end_year: int, archive_dates: list
):
    """
    ...
    for team in teams:
        ...
        for season in range(start_year, end_year + 1):
            logging.info(
                f"Attempting to determine depth charts for season {season} and for team {team['team']}"
            )

            # extract archives dictionary corresponding to current season
            seasonal_archives_dict = next(
                item for item in archive_dates if item["season"] == season
            )
            ...
            relevant_players = (
                set()
            )  # set containing players unique to following season
            ...
            player_depth_chart_position_records = (
                []
            )  # list of player depth chart position records
            ...
            for date, id in reversed(archives.items()):
                ...
                html = scraping_util.fetch_page(
                    f"https://www.ourlads.com/nfldepthcharts/archive/{id}/{(team['acronym'])}"
                )
                soup = BeautifulSoup(html, "html.parser")
                ...
                start_and_end_date_players_on_team(
                    relevant_players,
                    players,
                    start_date_mapping,
                    end_date_mapping,
...
def upsert_player_depth_chart_position_records(
    player_name_id_mapping: dict, player_depth_chart_position_records: list, season: int
):
    """
    ...
    logging.info(
        "Attempting to update relevant depth_chart_position records & insert new entries"
    )
    ...
    for record in player_depth_chart_position_records:
        ...
        if depth_chart_pos == record["depth_chart_pos"]:
            logging.info(
                f"Player '{record['name']}' has a valid player_depth_chart record persisted; skipping insertion/updates"
            )
            continue

        logging.info(
            f"Player '{record['name']}' has an INVALID player_depth_chart record persisted for Week {record['week']} and Season {record['season']}; adding record to list of records to update"
        )
    ...
    if records_to_insert:
        logging.info(
            f"'player_depth_chart' records to be inserted:\n\t{records_to_insert}"
        )
    ...
    if records_to_update:
        logging.info(
            f"'player_depth_chart' records to be updated:\n\t{records_to_update}"
        )
        update_player_depth_chart_postion(records_to_update)
    else:
        logging.info(
            f"No 'player_depth_chart' records require updating; skipping updates"
        )


def insert_player_depth_chart_position_records(
    player_name_id_mapping: dict, player_depth_chart_position_records: list, season: int
):
    """
    ...
    if not filtered_depth_chart_records:
        logging.info(
            f"No new player depth chart records in the {season} season; skipping insertion"
        )
    else:
        logging.info(
...
def generate_player_depth_chart_positions(
    players: list,
    date: str,
    date_week_mapping: dict,
    player_depth_chart_position_records: list,
    season: int,
    week: int = None,
):
    """
    ...
    for player in players:
        ...
        for week in range(start_week, end_week + 1):
            player_depth_chart_position_records.append(
                {"name": player_name, "week": week, "depth_chart_pos": depth_chart_pos}
            )


def upsert_player_teams_records(
    relevant_players: list,
    team_id: int,
    season: int,
    team: dict,
    player_name_id_mapping: dict,
    week: int,
):
    ...
    for player_name in relevant_players:
        ...
        player_team_records = fetch_player_teams_records_by_player_and_season(
            player_name_id_mapping[player_name], season
        )

        # if no player_teams record exist for player, add to list of records to be inserted
        if not player_team_records:
            player_teams_records_to_insert.append(
                {
                    "player_id": player_name_id_mapping[player_name],
                    "team_id": team_id,
                    "season": season,
                    "strt_wk": week,
                    "end_wk": 18,
        ...
        if found_valid_record:
            logging.info(
                f"Player '{player_name}' has a valid 'player_team' record persisted; no need to update/insert"
            )
            continue

        logging.info(
            f"Player '{player_name}' has no valid 'player_team' record persisted; must end date existing record & create new entry to account for new team"
        )

        # no record found, we must 1) end date relevant record 2) insert new record
        record_to_update = next(
            (record for record in player_team_records if record["end_wk"] == 18), None
        )
        if record_to_update is None:
            raise Exception(
                "No player_teams record with end_wk = 18 exists for player {player_name} for the {season} NFL Season; Unable to end date record and create new player_team record"
            )
    ...
    if player_teams_records_to_insert:
        logging.info(
            f"'player_teams' records to be inserted:\n\t{player_teams_records_to_insert}"
        )
    ...
    if player_teams_records_to_update:
        logging.info(
            f"'player_teams' records to be updated:\n\t{player_teams_records_to_update}"
        )
...
def insert_player_teams_records(
    relevant_players: list,
    start_date_mapping: dict,
    end_date_mapping: dict,
    team_id: int,
    season: int,
    team: dict,
    player_name_id_mapping: dict,
    ...
    filtered_player_teams_records = [
        ...
        if fetch_player_teams_record_by_pk(record) == None
    ]
    if not filtered_player_teams_records:
        logging.info(
            f"No new player teams records to insert for the team {team['team']} in the {season} season; skipping insertion"
        )
    else:
        print(
...
def insert_player_records(
    unique_player_records: list, team: dict, player_name_id_mapping: dict, season: dict
):
    """
    ...
    filtered_player_records = [
        record
        for record in unique_player_records
        if is_previously_inserted_player(record["name"], player_name_id_mapping)
        == False
    ]
    if not filtered_player_records:
        logging.info(
            f"No new player records to insert for the team {team['team']} in the {season} season; skipping insertion"
        )
    else:
        insert_players(filtered_player_records)


def start_and_end_date_players_on_team(
    relevant_players: list,
    ...
    if not relevant_players:
        ...
        for player in list(relevant_players):
            if (
                player not in curr_players and player not in end_date_mapping
            ):  # ensure we don't override previously end dated players
    ...
    if not unique_player_records:
        ...
        for player in curr_player_records:
            if not any(
                ...
                for existing_player in unique_player_records
            ):
                ...
                players.append(
                    {
                        "name": players_name,
                        "position": position,
                        "depth_chart_pos": depth_chart_pos,
                    }
                )
...
def extract_archive_dates(
    archive_dates_soup: BeautifulSoup, start_year: int, end_year: int
):
    """
    ...
    logging.info(
        f"Extracting relevant archive dates & ids for the years {start_year} - {end_year}"
    )
