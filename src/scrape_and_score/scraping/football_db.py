...
def scrape_historical(start_year: int, end_year: int):
    ...
    logging.info(
        f"Scraping & persisting player_injuries from the year {start_year} to the year {end_year}"
    )
...
def scrape_upcoming(week: int, season: int, player_ids: list = None):
    ...
    logging.info(
        f"Scraping & persisting player_injuries for week {week} for the {season} NFL season"
    )
    ...
    generate_and_persist_player_injury_records(
        player_injuries, season, week, player_ids
    )
...
def extract_player_statuses_and_injury(player: BeautifulSoup):
    ...
    injury_locations = (
        injury_location_div.get_text().strip().lower() if injury_location_div else None
    )

    # extract practice statuses
    practice_status_divs = player.find_all(
        "div", class_="td center w15 d-none d-md-table-cell"
    )
    wed_pract_status = (
        normalize_status(practice_status_divs[0].get_text())
        if len(practice_status_divs) > 0
        else None
    )
    thurs_pract_status = (
        normalize_status(practice_status_divs[1].get_text())
        if len(practice_status_divs) > 1
        else None
    )
    fri_pract_status = (
        normalize_status(practice_status_divs[2].get_text())
        if len(practice_status_divs) > 2
        else None
    )
...
def generate_and_persist_player_injury_records(
    ...
    persisted_player_injuries = fetch_pks_for_inserted_player_injury_records(
        season, week
    )
    ...
    for record in player_injuries:
        ...
        if player_id is None:
            logging.warn(
                f"Skipping record {record} for insertion since no player is persisted with normalized name {player_normalized_name}"
            )
            continue
        ...
        if player_ids is not None:
            if player_id not in player_ids:
                logging.warning(
                    f"Skipping record {record} for insertion/update since player has already played their game."
                )
                continue
    ...
    updated_records, insert_records = filter_persisted_records(
        persistable_records, persisted_player_injuries
    )

    if updated_records:
        logging.info(
            f"Attemtping to update {len(updated_records)} player_injuries records in database."
        )
        update_player_injuries(updated_records)

    if insert_records:
        logging.info(
            f"Attemtping to insert {len(insert_records)} player_injuries records into database."
        )
...
def filter_persisted_records(records: list, record_pks: list):
    ...
    logging.info(
        "Filtering player_injuries records to determine if they should be updated in database or inserted"
    )
