...
def preprocess(week: int = None, season: int = None):
    ...
    week_season_str = (
        f"for Week {week} of the {season} NFL Season"
        if week is not None and season is not None
        else ""
    )

    logging.info(
        f"Attempting to fetch & pre-process our training/testing data for our Neural Networks {week_season_str}..."
    )
    ...
    processed_df = pd.get_dummies(
        parsed_df, columns=["position"], dtype=int
    )  # encode categorical variable

    not_needed_cols = (
        ["player_id", "home_team"] if week is None and season is None else ["home_team"]
    )  # only remove player ID column in the case this isn't for generating predictions
    ...
    cyclical_df["max_week"] = cyclical_df["season"].apply(
        lambda season: 17 if season < 2021 else 18
    )
    cyclical_df["week_cos"] = np.cos(
        2 * np.pi * cyclical_df["week"] / cyclical_df["max_week"]
    )
    cyclical_df["week_sin"] = np.sin(
        2 * np.pi * cyclical_df["week"] / cyclical_df["max_week"]
    )
    cyclical_df = cyclical_df[
        ["week_cos", "week_sin"]
    ]  # only keep transformed features
...
def feature_selection(df: pd.DataFrame, position: str):
    ...
    logging.info(
        f"Attempting to determine relevant features for our {position} Neural Network Model"
    )
    ...
    for feature in feature_mapping.get(position):
        if feature not in features:
            logging.info(
                f"Manually adding the following feature to our {position} Neural Network Model: {feature}"
            )
            features.append(feature)

    logging.info(
        f"Selected Features via LassoCV and Manual Feature Selection: \n\n\t{features}"
    )
    return features
...
def manual_feature_engineering(df: pd.DataFrame):
    ...
    df["avg_wkly_pass_yds_per_att"] = df["avg_wkly_pass_yds"] / df[
        "avg_wkly_off_pass_att"
    ].replace(0, pd.NA)
...
def scale_and_transform(df: pd.DataFrame, return_inputs: bool = False):
    ...
    logging.info(
        "Scaling and transforming DF in order to utilize in Neural Network training & testing"
    )
    ...
    xs = (
        df.drop(columns=["fantasy_points"]).copy()
    ...
    for column in columns:
        if (
            column.startswith("precip_type")
            or column.startswith("weather_status")
            or column.startswith("surface")
        ):
    ...
    if return_inputs is False:
        ...
        logging.info(
            "return_inputs flag is true: Returning scaled values & transformed inputs"
        )
        return Xs, inputs
    ...
    for _, row in df.iterrows():
        ...
        for prop in week_props:
            ...
            if "(under)" in label:
                ...
                label = label.replace(
                    "_(over)", ""
                )  # remove over indicator since all lines/costs are over
...
def encode_game_conditions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["precip_type"] = (
        ...
        .replace("", None)
        .fillna("unknown")
        .map(normalize_game_condition)
    )
    df["surface"] = (
        df["surface"].replace("", None).fillna("unknown").map(normalize_game_condition)
    )
    df["weather_status"] = (
        df["weather_status"]
        .replace("", None)
    ...
    df["precip_probability"] = (
        df["precip_probability"].str.replace("%", "", regex=False).astype(float)
    )
...
def encode_player_injuries(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(
        "Encoding player injuries with relevant numerical values instead of statuses / strings"
    )
    ...
    practice_status_mapping = {"dnp": 0, "limited": 1, "full": 2}
    df["wednesday_practice_status"] = (
        df["wednesday_practice_status"].fillna("full").map(practice_status_mapping)
    )
    df["thursday_practice_status"] = (
        df["thursday_practice_status"].fillna("full").map(practice_status_mapping)
    )
    df["friday_practice_status"] = (
        df["friday_practice_status"].fillna("full").map(practice_status_mapping)
    )
    ...
    df["official_game_status"] = (
        df["official_game_status"].fillna("healthy").map(official_game_status_mapping)
    )
